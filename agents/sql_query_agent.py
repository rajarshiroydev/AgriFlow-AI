...
if settings.DATABASE_URL and sql_llm_for_chain_instance:
    try:
        ...
        with db_engine.connect() as connection_test: # Early connection test
            db_host_for_log = str(settings.DATABASE_URL).split('@')[-1] if '@' in str(settings.DATABASE_URL) else str(settings.DATABASE_URL)
            logger.info(f"Successfully connected to database for SQLDatabase wrapper init. Using URL ending with: ...{db_host_for_log}")
        ...
        logger.error(f"Failed to create SQLDatabase wrapper or test connection: {e}", exc_info=True)
        db_lc_wrapper = None
else:
...
if db_lc_wrapper and sql_llm_for_chain_instance and CUSTOM_SQL_PROMPT_OBJECT:
    try:
        sql_database_chain_instance = SQLDatabaseChain.from_llm(
            ...
            prompt=CUSTOM_SQL_PROMPT_OBJECT, verbose=True,
    ...
    try:
        sql_database_chain_instance = SQLDatabaseChain.from_llm(
            llm=sql_llm_for_chain_instance, db=db_lc_wrapper,
            verbose=True, return_intermediate_steps=True, top_k=10
        )
...
def extract_sql_from_llm_output(llm_output_text: str) -> Optional[str]:
    # This is the version from the debugging session that added more repr logs
    ...
    text_after_initial_marker_for_log = text_after_initial_marker 
    logger.debug(f"extract_sql_from_llm_output: Raw text after last '{sql_query_marker}' marker: '{text_after_initial_marker.replace(chr(10), r'\\n')[:300]}...'")
    ...
    if current_text_to_process.upper().startswith(sql_query_marker.upper()):
        ...
        logger.debug(f"extract_sql_from_llm_output: Text after stripping repeated '{sql_query_marker}': '{current_text_to_process.replace(chr(10), r'\\n')[:300]}...'")
    ...
    logger.debug(f"extract_sql_from_llm_output: Isolated text (potential SQL/NQP/explanation): '{sql_candidate_isolated.replace(chr(10), r'\\n')}'")
    ...
    if final_sql_candidate: 
        ...
        logger.warning(f"extract_sql_from_llm_output: No useful SQL/NQP/explanation text extracted. `final_sql_candidate` is empty. Original text after initial SQLQuery marker was '{text_after_initial_marker_for_log.replace(chr(10), r'\\n')[:100]}...'.")
        return None
...
def execute_natural_language_sql_query(user_query: str) -> Dict[str, Any]:
    ...
    try:
        ...
        if intermediate_steps and isinstance(intermediate_steps, list):
            # Pass 1: Look for a string element that contains SQLQuery, SQLResult, AND Answer (most complete)
            ...
            # Pass 2: If not found, look for a dict with 'sql_cmd' or 'llm_output' containing all markers
            if not llm_raw_output_for_sql:
                for step_content in intermediate_steps:
                    if isinstance(step_content, dict):
                        for key in ["sql_cmd", "llm_output", "statement", "query", "log"]: # Prioritize these keys
            ...
            # Pass 3: If still not found, look for just "SQLQuery:<SQL>" pattern (might be just the SQL)
            if not llm_raw_output_for_sql:
                # Check string elements first
                for step_content in intermediate_steps:
                ...
                if not llm_raw_output_for_sql: # Check specific dict keys
                    for step_content in intermediate_steps:
                        if isinstance(step_content, dict):
            ...
            # Pass 4: Last resort, the 'input' key from the first dictionary if it contains "SQLQuery:"
            # This was previously problematic if it was too short, so it's a low-priority fallback.
            if not llm_raw_output_for_sql:
                if intermediate_steps and isinstance(intermediate_steps[0], dict) and \
                   ...
                   "SQLQuery:" in intermediate_steps[0]["input"]:
                    # Only use if it seems to contain more than just "Question\nSQLQuery:"
                    # A rough heuristic: if it contains SQLResult or Answer, or is reasonably long.
        ...
        # Fallback to chain_response.get("query") if nothing from intermediate_steps
        ...
        if offending_sql.upper() == "NO_QUERY_POSSIBLE":
            ...
            if nl_answer and "No natural language answer processed" not in nl_answer and \
               ("cannot query" in nl_answer.lower() or "not possible" in nl_answer.lower() or "syntax error" in nl_answer.lower()):
                user_friendly_msg = nl_answer 
        ...
        if current_llm_raw_output:
             extracted_sql_check = extract_sql_from_llm_output(current_llm_raw_output)
